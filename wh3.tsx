...
const MultiValueContainer = (props: ValueContainerProps<OptionType>) => {
  ...
  const [visibleItems, setVisibleItems] = useState<OptionType[]>([]);

  const { selectProps: { value, menuIsOpen }, children } = props;
  const selectedItems = value as OptionType[];
  ...
  useEffect(() => {
    const calculateVisibleItems = () => {
      if (containerRef.current) {
        ...
        const maxVisibleItems: OptionType[] = [];

        // Loop through the selected items to measure their widths
        selectedItems.forEach((item, index) => {
          const itemElement = containerRef.current?.querySelector(
            `.custom-select__multi-value:nth-child(${index + 1})`
          ) as HTMLElement;

          if (itemElement) {
            accumulatedWidth += itemElement.offsetWidth;

            if (accumulatedWidth <= containerWidth) {
              maxVisibleItems.push(item);
            }
          }
        });

        setVisibleItems(maxVisibleItems);
        setIsOverflow(maxVisibleItems.length < selectedItems.length);
      }
    };

    // Only calculate if menu is closed
    if (!menuIsOpen) {
      calculateVisibleItems();
    } else {
      setIsOverflow(false); // Reset overflow when menu is open
    }
  ...
  return (
    <components.ValueContainer {...props}>
      <div ref={containerRef} className="multi-value-container">
        {!menuIsOpen && isOverflow ? (
          <div>
            {`${selectedItems.length} items selected`}
            <button
              className="h-0 inline-flex cursor-pointer items-center justify-center border-none bg-transparent p-0"
